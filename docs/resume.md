...
**Python · LangGraph · MCP · Anthropic/OpenAI SDK · asyncio · SQLite**
...
- LangGraph interrupt() 实现 Human-in-the-Loop 人工审核，Rich Live 暂停/恢复解决终端输入覆盖问题；SQLite checkpoint 实现崩溃恢复，`resume --last` 一键续跑
- operator.add reducer 实现跨轮次增量记忆（accumulated_knowledge / round_history / search_feedback / previous_draft），Writer 迭代修订替代每轮重写
- Critic 6 维度评分 + Supervisor 条件路由形成质量闭环；花括号深度计数解决 LLM 嵌套 JSON 解析失败

**难点**：quality_score 恒为 0（LLM 直接 Terminate）→ 增大步数 + 请求强指令；多轮迭代信息丢失 → 增量记忆机制；崩溃后无法续跑 → runs.json 持久化 + resume 命令
